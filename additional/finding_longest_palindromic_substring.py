...
def longestPalindrome(self, s: str) -> str:
    ''' 
    this function return the longest palindromic substring of s
    ...
    for j in range(len(s)):
        for k in range(j,len(s)):
            if dp[j][k]==1 and k-j+1>len(secret_word):
                secret_word=s[j:k+1]
        #clearly this cycle for can be optimized by started looking 
        #from the longest possible substrings and going 
        #toward the shortest, if a palindromic substring is find the cycle stops. 
        #For clarity in reading we chose the slowest but 
        #clearest approach
    return secret_word
